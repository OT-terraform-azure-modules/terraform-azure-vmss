...
resource "azurerm_linux_virtual_machine_scale_set" "linux" {
  ...
  dynamic "network_interface" {
    ...
    content {
      ...
      network_security_group_id     = var.network_security_group_id == [] ? null : var.network_security_group_id[count.index]   #lookup(network_interface.value, "network_security_group_id")
